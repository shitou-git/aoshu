...
<div class="container">
  ...
  <div class="tabs">
    <div class="home-btn-group">
      <button class="tab home-btn" onclick="showGame('silu')">📖 查看公式</button>
    </div>
...
export function initAoshu() {
    if (aoshuInited) return;
    aoshuInited = true;

    // 回车确认搜索（原 DOMContentLoaded 里的初始化代码）
    ["easy", "medium", "hard"].forEach(level => {
        const input = document.getElementById("search-" + level);
        if (input) {
    ...
    // 加载题库（用 aoshuDataLoaded 标记防止重复加载）
    if (!aoshuDataLoaded) {
